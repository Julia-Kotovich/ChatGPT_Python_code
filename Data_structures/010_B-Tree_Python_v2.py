class BTreeNode:
    ...
    def split_child(self, index):
        ...
        self.keys.insert(index, split_node.keys.pop(2))
        new_node.keys = split_node.keys[2:]
        split_node.keys = split_node.keys[:2]

        if not split_node.leaf:
            new_node.child = split_node.child[2:]
            split_node.child = split_node.child[:2]
